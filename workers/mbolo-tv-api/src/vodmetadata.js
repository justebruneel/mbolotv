...
// Synopsis + backdrop d'un titre VOD (cache-first, miss persisté). Retourne
// null si le titre est absent de TVmaze — jamais d'erreur bloquante.
export async function vodMetadata(env, rawTitle, kind = 'SERIES') {
  ...
  } catch { /* cache indisponible : on interroge TVmaze */ }

  let payload;
  try {
    payload = await fetchTvmaze(clean);
  } catch {
    return null;
  }
