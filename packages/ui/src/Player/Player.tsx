...
export function Player({ urls, title, initialVolume, initialLevel, initialDataSaver, onVolumeChange, onLevelChange, onDataSaverChange }: PlayerProps) {
  ...
  useEffect(() => {
    ...
    setStatus('loading'); setBuffering(false); setLevels([]); setActiveLevel(-1); setAutoplayBlocked(false); setRetrying(false); setStats({ startupMs: null, rebufferCount: 0, bufferAhead: 0, bitrate: null, latency: null });
    ...
    function loadCurrent(): void {
      ...
      const hls = new Hls({ debug: DEBUG, enableWorker: true, lowLatencyMode: false, backBufferLength: 15, maxBufferLength: 25, maxMaxBufferLength: 45, maxBufferSize: 50 * 1000 * 1000, maxBufferHole: 0.5, liveSyncDurationCount: 3, liveMaxLatencyDurationCount: 8, startLevel: -1, abrEwmaDefaultEstimate: 1_500_000, abrBandWidthFactor: 0.8, abrBandWidthUpFactor: 0.6, abrMaxWithRealBitrate: true, capLevelToPlayerSize: true, maxLoadingDelay: 4, maxFragLookUpTolerance: 0.5, manifestLoadingTimeOut: 12_000, manifestLoadingMaxRetry: 2, levelLoadingTimeOut: 12_000, levelLoadingMaxRetry: 2, fragLoadingTimeOut: 18_000, fragLoadingMaxRetry: 3, maxStarvationDelay: 8 });
      hlsRef.current = hls; retryRef.current = loadCurrent; hls.loadSource(urls[urlIndex]); hls.attachMedia(el);
      hls.on(Hls.Events.ERROR, (_event, data: ErrorData) => { if (cancelled || !data.fatal) return; if (data.type === ErrorTypes.MEDIA_ERROR) { if (el.buffered.length > 0 && el.currentTime + 0.75 < el.buffered.end(el.buffered.length - 1)) el.currentTime += 0.75; hls.recoverMediaError(); return; } if (data.type === ErrorTypes.NETWORK_ERROR) { const code = data.response?.code; const master = typeof data.url === 'string' && data.url.includes('master.m3u8'); if (code === 401 || code === 403 || (code === 404 && master)) { if (urlIndex + 1 < urls.length) { urlIndex += 1; retries = 0; networkRetries = 0; loadCurrent(); } else setStatus('error'); return; } if ([Hls.ErrorDetails.MANIFEST_LOAD_ERROR, Hls.ErrorDetails.MANIFEST_LOAD_TIMEOUT, Hls.ErrorDetails.LEVEL_LOAD_ERROR, Hls.ErrorDetails.LEVEL_LOAD_TIMEOUT].includes(data.details)) { advance(); return; } networkRetries += 1; if (networkRetries <= MAX_NETWORK_RETRIES) { retryTimer = setTimeout(() => { if (!cancelled && hlsRef.current === hls) hls.startLoad(); }, Math.min(1000 * networkRetries, 3000)); return; } } advance(); });
