...
function networkProfile(): { estimate: number; capHeight: number | null; buffer: number } {
  ...
  if (connection?.saveData || type === 'slow-2g' || type === '2g' || downlink > 0 && downlink < 1) return { estimate: 350_000, capHeight: 360, buffer: 8 };
...
export function Player({ urls, title, initialVolume, initialLevel, initialDataSaver, onVolumeChange, onLevelChange, onDataSaverChange }: PlayerProps) {
  ...
  useEffect(() => {
    ...
    setStatus('loading'); setBuffering(false); setLevels([]); setActiveLevel(-1); setAutoplayBlocked(false); setRetrying(false); setStats({ startupMs: null, rebufferCount: 0, latency: null, bitrate: null, rebufferCount: 0 });
    ...
    function loadCurrent(): void {
      ...
      const hls = new Hls({ debug: DEBUG, enableWorker: true, lowLatencyMode: false, startFragPrefetch: true, backBufferLength: 6, maxBufferLength: profile.buffer, maxMaxBufferLength: 24, maxBufferSize: 24 * 1000 * 1000, maxBufferHole: 0.5, liveSyncDurationCount: 5, liveMaxLatencyDurationCount: 12, startLevel: 0, abrEwmaDefaultEstimate: profile.estimate, abrEwmaFastVoD: 2, abrEwmaSlowVoD: 5, abrBandWidthFactor: 0.7, abrBandWidthUpFactor: 0.5, abrMaxWithRealBitrate: true, capLevelToPlayerSize: true, maxLoadingDelay: 2, maxFragLookUpTolerance: 0.3, manifestLoadingTimeOut: 15_000, manifestLoadingMaxRetry: 3, manifestLoadingRetryDelay: 1000, levelLoadingTimeOut: 15_000, levelLoadingMaxRetry: 3, levelLoadingRetryDelay: 1000, fragLoadingTimeOut: 20_000, fragLoadingMaxRetry: 4, fragLoadingRetryDelay: 1000, maxStarvationDelay: 4 });
