  ...
  return (
    <div className={styles.shell}>
      <header className={styles.topbar}>
        <button type="button" className={styles.menuButton} onClick={() => setOpen(true)} aria-label="Ouvrir le menu" aria-expanded={open}><Menu size={20} aria-hidden /></button>
        <span className={styles.topbarBrand}>{brand}</span>
        <nav className={styles.utilityNav} aria-label="Liens utiles">{utilityItems.map((item) => <Link key={item.href} href={item.href} className={styles.utilityLink}>{item.label}</Link>)}</nav>
      </header>
      ...
      <aside className={[styles.sidebar, open ? styles.sidebarOpen : ''].filter(Boolean).join(' ')} aria-label="Navigation principale">
        ...
        <nav className={styles.nav}>
          {navItems.map((item) => { const active = item.href === activeHref; return <Link key={item.href} href={item.href} className={[styles.item, active ? styles.active : ''].filter(Boolean).join(' ')} aria-current={active ? 'page' : undefined}>{item.icon && <span className={styles.icon}>{item.icon}</span>}<span>{item.label}</span></Link>; })}
        </nav>
        <div className={styles.secondaryNav}>
          <p className={styles.sectionLabel}>Ressources</p>
          {utilityItems.map((item) => <Link key={item.href} href={item.href} className={styles.item}><span className={styles.icon}>{item.icon ?? <ExternalLink size={17} />}</span><span>{item.label}</span></Link>)}
          {installEvent && <button type="button" className={styles.installButton} onClick={() => void install()}><Download size={16} /> Installer l’application</button>}
        </div>
        <div className={styles.sidebarFooter}>© {new Date().getFullYear()} Mbolo TV<br /><span>Créer par Groupe Nzogho</span></div>
      </aside>
