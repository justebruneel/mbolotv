...
export function AppShell({ brand, navItems, utilityItems = [], activeHref, pathname, children }: AppShellProps) {
  ...
  return <div className={styles.shell}>
    <header className={styles.topbar}><button type="button" className={styles.menuButton} onClick={() => setOpen(true)} aria-label="Ouvrir le menu" aria-expanded={open}><Menu size={20} aria-hidden /></button><span className={styles.topbarBrand}>{brand}</span><nav className={styles.utilityNav} aria-label="Liens utiles">{utilityItems.map((item) => <Link key={item.href} href={item.href} className={styles.utilityLink}>{item.label}</Link>)}</nav></header>
    ...
    <main className={styles.main}><header className={styles.desktopHeader}><div><p className={styles.eyebrow}>Mbolo TV</p><p className={styles.headerTitle}>Votre télévision, plus simple.</p></div><nav aria-label="Navigation ressources">{utilityItems.map((item) => <Link key={item.href} href={item.href} className={styles.desktopLink}>{item.label}</Link>)}</nav></header>{children}</main>
  </div>;
}
