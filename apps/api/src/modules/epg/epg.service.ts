...
export class EpgService {
  ...
  async search(query: ProgrammeSearchQuery): Promise<ProgrammeSearchResponse> {
    const where = { title: { contains: query.q }, channel: { isVisible: true, variants: { some: { isActive: true } }, ...(await this.hiddenCategoryClause()), ...(query.category ? { category: { slug: query.category } } : {}) } };
