...
export class EpgImportService {
  ...
  async run(): Promise<EpgImportResult> {
    ...
    const sources = await this.prisma.source.findMany({ where: { kind: 'XTREAM', status: { not: 'DISABLED' } }, orderBy: [{ priority: 'asc' }] }) as EpgSource[];
    ...
    for (const source of sources) {
      try {
        const connection = JSON.parse(this.crypto.decrypt(source.connectionEncrypted)) as Record<string, string>; const url = this.buildXmltvUrl(connection); if (!url) continue;
