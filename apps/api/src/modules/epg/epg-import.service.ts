...
type EpgRow = { channelId: string; startsAt: Date; endsAt: Date; title: string; description?: string; metadata?: { categories: string[] } };
...
export class EpgImportService {
  ...
  async run(): Promise<EpgImportResult> {
    ...
    const sources = await this.prisma.source.findMany({ where: { kind: 'XTREAM', status: { not: 'DISABLED' } }, orderBy: [{ priority: 'asc' }] });
    ...
    let channels = 0; let programmes = 0; let stored = 0; let sourcesDone = 0;
    for (const source of sources) {
      try {
        const connection = JSON.parse(this.crypto.decrypt(source.connectionEncrypted)) as Record<string, string>;
        const url = this.buildXmltvUrl(connection); if (!url) continue;
        const result = await fetcher.fetchStream(url, { maxBytes, streamTimeoutMs: 15 * 60_000, userAgent: 'MboloTV/0.1 (EPG import)' });
        if (!result.ok || !result.stream) { this.logger.warn(`EPG indisponible pour ${source.name}: ${result.error}`); continue; }
        ...
        const parseResult = await parseXmltvStream(result.stream, async (batch: XmltvProgramme[]) => {
          ...
          for (const programme of batch) {
            const channelId = tvgMap.get(programme.channelId.toLowerCase()); if (!channelId) continue;
            matched.add(channelId);
            rows.push({ channelId, startsAt: programme.startsAt, endsAt: programme.endsAt, title: programme.title, description: programme.description, metadata: programme.categories.length > 0 ? { categories: programme.categories } : undefined });
          }
          if (rows.length === 0) return 0;
          const created = await this.prisma.epgProgramme.createMany({ data: rows }); return created.count;
        });
        inserted = parseResult.stored; channels += matched.size; programmes += parseResult.programmes; stored += inserted; sourcesDone += 1;
        await this.prisma.source.update({ where: { id: source.id }, data: { lastSyncedAt: new Date() } });
        this.logger.log(`EPG ${source.name}: ${matched.size} chaînes, ${inserted} programmes sur ${parseResult.programmes}`);
  ...
  private async buildTvgMap(): Promise<Map<string, string>> {
    const channels = await this.prisma.channel.findMany({ where: { tvgId: { not: null } }, select: { id: true, tvgId: true } });
    const map = new Map<string, string>(); for (const channel of channels) if (channel.tvgId) map.set(channel.tvgId.toLowerCase(), channel.id); return map;
  }
