...
const DEDUP_BUCKET_MS = 3 * 3_600_000; const POSTPONED_GRACE_MS = 2 * 3_600_000; const FINISHED_RETENTION_MS = 24 * 3_600_000; // Durée max plausible d'un match : au-delà, un LIVE sans endsAt est clos.
const MAX_LIVE_WITHOUT_END_MS = 3 * 3_600_000;
...
export class MatchesDiscoveryService {
  ...
  private async refreshStates(now: Date): Promise<number> { const live = await this.prisma.match.updateMany({ where: { state: 'SCHEDULED', startsAt: { lte: now }, endsAt: { gt: now } }, data: { state: 'LIVE' } }); const finished = await this.prisma.match.updateMany({ where: { state: { in: ['SCHEDULED', 'LIVE', 'POSTPONED'] }, endsAt: { lt: now } }, data: { state: 'FINISHED' } }); // LIVE sans fin (endsAt NULL) : clos après 3 h de jeu, sinon il resterait
    // « en direct » pour toujours dans « Sport en direct ».
    const orphanLive = await this.prisma.match.updateMany({ where: { state: 'LIVE', endsAt: null, startsAt: { lt: new Date(now.getTime() - MAX_LIVE_WITHOUT_END_MS) } }, data: { state: 'FINISHED' } }); const grace = new Date(now.getTime() - POSTPONED_GRACE_MS); const postponed = await this.prisma.match.updateMany({ where: { state: 'SCHEDULED', startsAt: { lt: grace }, endsAt: { gte: now } }, data: { state: 'POSTPONED' } }); return live.count + finished.count + orphanLive.count + postponed.count; }
  private async purgeFinished(): Promise<number> { const cutoff = new Date(Date.now() - FINISHED_RETENTION_MS); const result = await this.prisma.match.deleteMany({ where: { state: 'FINISHED', OR: [{ endsAt: { lt: cutoff } }, { endsAt: null }] } }); return result.count; }
}
