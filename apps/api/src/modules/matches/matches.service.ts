...
interface MatchRow { id: string; sport: string; competition: string; homeTeam: string; awayTeam: string; startsAt: Date; endsAt: Date | null; state: string; homeTeamLogo?: string | null; awayTeamLogo?: string | null; matchStreams: Array<{ streamVariant: { id: string; healthScore: number; healthStatus: string | null; isActive: boolean; encryptedLocator: Uint8Array; sourceId: string; channel: { id: string; name: string; logoKey: string | null } } }>; }
...
export class MatchesService {
  ...
  async list(query: MatchQuery): Promise<MatchListResponse> {
    ...
    return { items: rows.map((row: MatchRow) => this.serialize(this.toMatchWithVariants(row, sources), { home: row.homeTeamLogo, away: row.awayTeamLogo })), total: rows.length };
  }
  async findOne(id: string): Promise<Match> { const found = await this.findMatchWithBadges(id); return this.serialize(found.match, found.badges); }
  ...
  private async findMatchWithBadges(id: string): Promise<{ match: MatchWithVariants; badges: { home: string | null; away: string | null } }> { const row = await this.prisma.match.findUnique({ where: { id }, include: { matchStreams: { include: { streamVariant: { select: VARIANT_SELECT } } } } }) as unknown as MatchRow | null; if (!row) throw new NotFoundException('Match not found'); const sources = await this.loadSources([row]); return { match: this.toMatchWithVariants(row, sources), badges: { home: row.homeTeamLogo ?? null, away: row.awayTeamLogo ?? null } }; }
  ...
  private serialize(match: MatchWithVariants, badges?: { home?: string | null; away?: string | null }): Match { const byChannel = new Map<string, MatchChannel>(); for (const variant of match.variants) { if (!variant.isActive || variant.source?.status === 'DISABLED') continue; const channel = variant.channel; const entry = byChannel.get(channel.id); if (entry) { entry.streamCount += 1; entry.bestHealth = Math.max(entry.bestHealth ?? 0, variant.healthScore); } else byChannel.set(channel.id, { id: channel.id, name: channel.name, logoUrl: channel.logoKey, streamCount: 1, bestHealth: variant.healthScore }); } return { id: match.id, sport: match.sport, competition: match.competition, homeTeam: match.homeTeam, awayTeam: match.awayTeam, startsAt: match.startsAt.toISOString(), endsAt: match.endsAt ? match.endsAt.toISOString() : null, state: match.state as Match['state'], homeTeamLogo: badges?.home ?? null, awayTeamLogo: badges?.away ?? null, channels: [...byChannel.values()] }; }
}
