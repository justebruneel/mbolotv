...
export class SourcesService {
  ...
  async replacePlaylistStream(ownerId: string, id: string, input: Readable): Promise<SourceResponse> {
    ...
    const upload = this.storage.putStream(key, pass, 'application/x-mpegurl', UPLOAD_MAX_BYTES);
    let head = Buffer.alloc(0); let bytes = 0;
    try {
      for await (const chunk of input) {
        const buffer = Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk);
        bytes += buffer.byteLength;
        if (bytes > UPLOAD_MAX_BYTES) throw new PayloadTooLargeException('Fichier trop volumineux');
        if (head.byteLength < 512) head = Buffer.concat([head, buffer.subarray(0, 512 - head.byteLength)]);
        if (!pass.write(buffer)) await new Promise<void>((resolveDrain) => pass.once('drain', resolveDrain));
      }
      pass.end();
      await upload;
    } catch (error) { pass.destroy(); await this.storage.delete(key); throw error; }
    if (!head.toString('utf8').trimStart().startsWith('#EXTM3U')) { await this.storage.delete(key); throw new BadRequestException('En-tête #EXTM3U manquant'); }
    const updated = await this.prisma.source.update({ where: { id }, data: { connectionEncrypted: this.crypto.encrypt(JSON.stringify({ filePath: key })), status: 'PENDING' } });
    await this.audit.log(ownerId, 'source.upload_playlist', 'source', id, { bytes });
    await this.startImport(ownerId, id, false);
  ...
  private buildProbe(kind: string, connection: Record<string, string>): string | null { if (kind === 'M3U') return connection['url'] ?? connection['playlistUrl'] ?? null; if (kind === 'XTREAM') { const host = connection['host'] ?? connection['url']; if (!host || !connection['username'] || !connection['password']) return null; return `${host.replace(/\/+$/, '')}/player_api.php?username=${encodeURIComponent(connection['username'])}&password=${encodeURIComponent(connection['password'])}`; } if (kind === 'MAC_PORTAL') { const portal = connection['portal'] ?? connection['url']; return portal ? `${portal.replace(/\/+$/, '')}/portal.php` : null; } return null; }
  private maskValue(value: string): string { if (!value) return '••••'; const visible = value.replace(/^https?:\/\//, '').slice(0, 4); return value.length <= 8 ? '••••' : `${visible}…`; }
  private async findOwned(ownerId: string, id: string) { const source = await this.prisma.source.findFirst({ where: { id, ownerId } }); if (!source) throw new NotFoundException('Source introuvable'); return source; }
  private serialize(source: { id: string; name: string; kind: string; status: string; priority: number; lastSyncedAt: Date | null; createdAt: Date }): SourceResponse { return { id: source.id, name: source.name, kind: source.kind as SourceResponse['kind'], status: source.status as SourceResponse['status'], priority: source.priority, lastSyncedAt: source.lastSyncedAt?.toISOString() ?? null, createdAt: source.createdAt.toISOString() }; }
