...
export class SourcesService {
  ...
  async list(ownerId: string): Promise<SourceResponse[]> {
    const sources = await this.prisma.source.findMany({ where: { ownerId }, orderBy: [{ priority: 'asc' }, { createdAt: 'desc' }] });
  ...
  async create(ownerId: string, input: SourceCreateInput): Promise<SourceResponse> {
    ...
    const source = await this.prisma.source.create({ data: { ownerId, name: input.name, kind: input.kind, connectionEncrypted: encrypted, status: 'PENDING' } });
    await this.audit.log(ownerId, 'source.create', 'source', source.id, { kind: input.kind, name: input.name });
    if (input.kind === 'M3U' && (input.connection['url'] || input.connection['playlistUrl'])) await this.startImport(ownerId, source.id, { silentAudit: true });
  ...
  async update(ownerId: string, id: string, input: SourceUpdateInput): Promise<SourceResponse> {
    ...
    const updated = await this.prisma.source.update({ where: { id }, data: { name: input.name, priority: input.priority, status: input.status } });
  ...
  async importNow(ownerId: string, id: string): Promise<ImportRun> { return this.startImport(ownerId, id); }
  ...
  async listImports(ownerId: string): Promise<ImportRunListResponse> {
    const runs = await this.prisma.importRun.findMany({ where: { source: { ownerId } }, include: { source: true }, orderBy: { startedAt: 'desc' }, take: 100 });
  ...
  async importDetail(ownerId: string, id: string): Promise<ImportRun> {
    const run = await this.prisma.importRun.findFirst({ where: { id, source: { ownerId } }, include: { source: true } });
  ...
  private async startImport(ownerId: string, sourceId: string, options?: { silentAudit?: boolean }): Promise<ImportRun> {
    const source = await this.prisma.source.findFirst({ where: { id: sourceId, ownerId } });
  ...
  private serialize(source: { id: string; name: string; kind: string; status: string; priority: number; lastSyncedAt: Date | null; createdAt: Date }, _unused?: null, extra?: Partial<SourceDetail>): SourceResponse & Partial<SourceDetail> {
    return { id: source.id, name: source.name, kind: source.kind as SourceResponse['kind'], status: source.status as SourceResponse['status'], priority: source.priority, lastSyncedAt: source.lastSyncedAt?.toISOString() ?? null, createdAt: source.createdAt.toISOString(), ...extra };
  }
  ...
  private serializeRun(run: { id: string; sourceId: string; state: string; metrics: unknown; errorCode: string | null; errorMessage: string | null; startedAt: Date; completedAt: Date | null; source?: { name: string } }): ImportRun {
    return { id: run.id, sourceId: run.sourceId, sourceName: run.source?.name ?? 'source supprimée', state: run.state as ImportRun['state'], metrics: run.metrics as ImportRun['metrics'], errorCode: run.errorCode, errorMessage: run.errorMessage, startedAt: run.startedAt.toISOString(), completedAt: run.completedAt?.toISOString() ?? null };
  }
}
