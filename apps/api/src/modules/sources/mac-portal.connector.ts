...
function arrayFrom<T>(value: T[] | { data?: T[]; results?: T[]; items?: T[] } | undefined): T[] { if (Array.isArray(value)) return value; return value?.data ?? value?.results ?? value?.items ?? []; }
...
export async function fetchMacPortalEntries(connection: MacPortalConnection): Promise<{ entries: ParsedChannel[] }> {
  ...
  const headers = { MAC: mac, Cookie: `mac=${mac};stb_lang=en;timezone=UTC`, Accept: 'application/json, text/javascript, */*; q=0.01', 'X-User-Agent': 'Model: MAG254; Link: Ethernet', Referer: `${base}/`, Origin: new URL(base).origin };
  const { token, endpoint } = await getPortalToken(fetcher, portalEndpoints(base), headers, connection.signal);
  const genresById = await getGenres(fetcher, endpoint, token, headers, connection.signal);
  const channelsResult = await fetcher.fetch(`${endpoint}/portal.php?type=itv&action=get_all_channels&token=${encodeURIComponent(token)}&JsHttpRequest=1-json`, { maxBytes: MAX_API_BYTES, headers: { ...headers, Authorization: `Bearer ${token}` }, timeoutMs: CONNECTOR_TIMEOUT_MS, signal: connection.signal });
  ...
  return { entries: channels.filter((channel) => channel.id != null).map((channel) => { const rawGenres = Array.isArray(channel.genres) ? channel.genres : channel.genres != null ? [channel.genres] : channel.genre_id != null ? [channel.genre_id] : []; const genre = rawGenres.length ? genresById.get(Number(rawGenres[0])) : undefined; return { title: String(channel.name ?? `Chaîne ${channel.number ?? channel.id}`).trim(), tvgLogo: channel.logo || undefined, groupTitle: genre, url: `${playbackBase}/play/live/${token}/${channel.id}.ts` }; }) };
}

function isM3uProvisioningUrl(value: string): boolean { try { const url = new URL(value); return /(?:^|\/)get\.php$/i.test(url.pathname) || ['m3u', 'm3u_plus'].includes((url.searchParams.get('type') ?? '').toLowerCase()); } catch { return false; } }
...
function normalizePlaybackBase(value: string): string { const url = new URL(value); url.search = ''; if (/\/portal\.php$/i.test(url.pathname)) url.pathname = url.pathname.replace(/\/portal\.php$/i, ''); return url.toString().replace(/\/$/, ''); }
function portalEndpoints(base: string): string[] { const url = new URL(base); const origin = url.origin; return Array.from(new Set([base, `${origin}/stalker_portal/server`, `${origin}/stalker_portal`, origin].map((value) => value.replace(/\/$/, '')))); }
...
async function getPortalToken(fetcher: SafeFetcher, endpoints: string[], headers: Record<string, string>, signal?: AbortSignal): Promise<{ token: string; endpoint: string }> {
  ...
  for (const endpoint of endpoints) {
    ...
    try { const payload = JSON.parse(handshake.body) as { js?: { token?: string } }; if (payload.js?.token) return { token: payload.js.token, endpoint }; if (payload.js) throw new Error('Aucun token Stalker'); } catch (error) { if (error instanceof Error && error.message === 'Aucun token Stalker') throw error; lastError = 'Handshake Stalker invalide'; }
  }
...
async function getGenres(fetcher: SafeFetcher, endpoint: string, token: string, headers: Record<string, string>, signal?: AbortSignal): Promise<Map<number, string>> {
  ...
  try { const result = await fetcher.fetch(`${endpoint}/portal.php?type=itv&action=get_genres&token=${encodeURIComponent(token)}&JsHttpRequest=1-json`, { headers: { ...headers, Authorization: `Bearer ${token}` }, timeoutMs: CONNECTOR_TIMEOUT_MS, signal }); if (!result.ok || !result.body) return map; const payload = JSON.parse(result.body) as StalkerEnvelope<StalkerGenre>; for (const genre of arrayFrom(payload.js)) if (genre.id != null && genre.title) map.set(Number(genre.id), String(genre.title).trim()); } catch { /* genres facultatifs */ }
  return map;
}
