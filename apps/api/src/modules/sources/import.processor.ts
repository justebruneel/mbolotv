...
export class ImportProcessor implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    // Le worker dédié reste optionnel pour l’instant. Traiter la queue ici dans
    // les deux modes évite que les imports BullMQ restent en QUEUED quand le
    // processus worker n’est pas lancé. En production multi-instance, une seule
    // instance API doit activer ce consommateur, ou ce traitement devra être
    // déplacé dans un worker partagé avec verrou distribué.
  ...
  private async importM3u(source: Source, connection: Record<string, string>, importRunId: string): Promise<ImportMetrics> {
    ...
    const result = await new SafeFetcher().fetchStream(url, { maxBytes, streamTimeoutMs: timeoutMs });
    if (!result.ok || result.stream === undefined) throw new ImportError('FETCH_ERROR', result.error ?? 'Échec de téléchargement');
  ...
  private async probeSubplaylistEntries(entries: ParsedChannel[]): Promise<ParsedChannel[]> {
    ...
    for (const entry of entries) {
      ...
      if (!result.ok || result.body === undefined || !isSubplaylistContainer(result.body)) kept.push(entry);
    }
    return kept;
  }
  ...
  private async importXtream(source: Source, connection: Record<string, string>, importRunId: string): Promise<ImportMetrics> {
    ...
    if (!url || !username || !password) throw new ImportError('MISSING_CREDENTIALS', 'Identifiants Xtream manquants');
    await this.prisma.importRun.update({ where: { id: importRunId }, data: { state: 'PARSING' } });
    try {
  ...
  private async importMacPortal(source: Source, connection: Record<string, string>, importRunId: string): Promise<ImportMetrics> {
    const url = connection['url'];
  ...
  private async ingestEntries(source: Source, entries: ParsedChannel[], importRunId: string): Promise<ImportMetrics> {
    ...
    for (const entry of entries) {
      try {
        position += 1;
        if (position % 2_000 === 0) await this.prisma.importRun.update({ where: { id: importRunId }, data: { metrics: { phase: 'NORMALIZING', processed: position, total: entries.length } as unknown as Prisma.InputJsonValue } });
        const key = slugify(entry.title);
        if (seenKeys.has(key)) { metrics.duplicates += 1; continue; }
        seenKeys.add(key);
        ...
        if (entry.tvgLogo && /^https?:\/\//i.test(entry.tvgLogo)) logoUrlsByKey.set(key, entry.tvgLogo);
        const existingChannel = channelByKey.get(key);
        if (!existingChannel) {
          channelCreates.push({ name: entry.title, canonicalName: entry.title, normalizedKey: key, tvgId: entry.tvgId ?? null, country, categorySlug, sortOrder: position });
        } else {
          const update: PendingUpdate = { id: existingChannel.id, sortOrder: position };
          let changed = position !== existingChannel.sortOrder;
          if (entry.tvgId && entry.tvgId !== existingChannel.tvgId) { update.tvgId = entry.tvgId; changed = true; }
          if (country && country !== existingChannel.country) { update.country = country; changed = true; }
          if (categorySlug) {
            const resolvedCategoryId = categoryBySlug.get(categorySlug)?.id ?? null;
            if (resolvedCategoryId !== existingChannel.categoryId) { update.categorySlug = categorySlug; changed = true; }
          }
        ...
        if (existingVariant) {
          let same = true;
          try { same = this.crypto.decrypt(existingVariant.encryptedLocator) === entry.url; } catch { same = false; }
          if (!same) variantUpdates.push({ id: existingVariant.id, encryptedLocator: this.crypto.encrypt(entry.url) });
        } else newVariantsByKey.set(key, entry.url);
      } catch { metrics.errors += 1; }
    }

    for (const [index, slug] of newCategorySlugs.entries()) {
      categoryBySlug.set(slug, await this.prisma.category.create({ data: { slug, name: slug, sortOrder: categoryOrders.get(slug) ?? existingCategories.length + index } }));
    }
    ...
    if (channelCreates.length > 0) {
      ...
      for (let i = 0; i < channelCreates.length; i += chunkSize) {
        ...
        await this.prisma.channel.createMany({ data: chunk.map((c) => ({ name: c.name, canonicalName: c.canonicalName, normalizedKey: c.normalizedKey, tvgId: c.tvgId, country: c.country, categoryId: c.categorySlug ? (categoryBySlug.get(c.categorySlug)?.id ?? null) : null, sortOrder: c.sortOrder })) });
      }
      const createdKeys = channelCreates.map((c) => c.normalizedKey);
      for (let i = 0; i < createdKeys.length; i += 10_000) {
        const created = await this.prisma.channel.findMany({ where: { normalizedKey: { in: createdKeys.slice(i, i + 10_000) } } });
    ...
    if (newVariantsByKey.size > 0) {
      ...
      for (const [key, url] of newVariantsByKey) {
        ...
        if (!channel) { metrics.errors += 1; continue; }
        variants.push({ channelId: channel.id, sourceId: source.id, encryptedLocator: this.crypto.encrypt(url) });
      }
      for (let i = 0; i < variants.length; i += 5_000) await this.prisma.streamVariant.createMany({ data: variants.slice(i, i + 5_000) });
    ...
    if (channelUpdates.length > 0 || variantUpdates.length > 0) {
      ...
      const flush = async (): Promise<void> => { if (ops.length > 0) await this.prisma.$transaction(ops.splice(0)); };
      ...
      for (const update of variantUpdates) {
        ops.push(this.prisma.streamVariant.update({ where: { id: update.id }, data: { encryptedLocator: update.encryptedLocator, isActive: true } }));
        if (ops.length >= 1_000) await flush();
      }
      await flush();
    ...
    if (entries.length > 0) {
      ...
      for (let i = 0; i < toPrune.length; i += 500) {
        const result = await this.prisma.streamVariant.updateMany({ where: { id: { in: toPrune.slice(i, i + 500).map((variant) => variant.id) } }, data: { isActive: false } });
  ...
  private async storeChannelLogos(logoUrlsByKey: Map<string, string>, channelByKey: Map<string, { id: string; logoKey: string | null }>): Promise<number> {
    if (logoUrlsByKey.size === 0) return 0;
    const uniqueUrls = Array.from(new Set(logoUrlsByKey.values())).slice(0, this.logoMaxDownloads);
    const fetcher = new SafeFetcher();
    ...
    for (const [key, url] of logoUrlsByKey) {
      ...
      if (channel && storageKey && channel.logoKey !== storageKey) updates.push({ id: channel.id, logoKey: storageKey });
    }
    for (let i = 0; i < updates.length; i += 1_000) {
      await this.prisma.$transaction(updates.slice(i, i + 1_000).map((update) => this.prisma.channel.update({ where: { id: update.id }, data: { logoKey: update.logoKey } })));
    }
    return urlToKey.size;
  }

  private async storeOneLogo(fetcher: SafeFetcher, url: string): Promise<string | null> {
    const result = await fetcher.fetchStream(url, { maxBytes: this.logoMaxBytes, streamTimeoutMs: this.logoTimeoutMs });
    if (!result.ok || result.stream === undefined || !(result.contentType ?? '').startsWith('image/')) return null;
  ...
  private async fail(sourceId: string, importRunId: string, errorCode: string, error: unknown): Promise<void> {
    ...
    await this.prisma.importRun.update({ where: { id: importRunId }, data: { state: 'FAILED', errorCode, errorMessage: this.sanitizeMessage(message, 300), completedAt: new Date() } });
  ...
  private sanitizeMessage(message: string, maxLength: number): string {
    return message.replace(/https?:\/\/[^\s]+/g, '[url masquée]').slice(0, maxLength);
  }
}

export type { SourceKind };

function collectStream(stream: ReadableStream<Uint8Array>, maxBytes: number): Promise<Buffer | null> {
  return (async () => {
