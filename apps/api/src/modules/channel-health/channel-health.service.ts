...
export class HealthCheckService {
  ...
  isStale(checkedAt: Date | null): boolean {
    if (!checkedAt) return true;
    return Date.now() - checkedAt.getTime() > this.staleMs;
  }
  ...
  async checkVariant(variant: VariantLike): Promise<HealthStatus> {
    ...
    try {
      const result = await this.fetcher.fetch(providerUrl, {
        maxBytes: MAX_MASTER_BYTES,
        timeoutMs: this.timeoutMs,
        userAgent: BROWSER_USER_AGENT,
      });
      status = result.ok && result.status >= 200 && result.status < 400 ? 'OK' : 'DOWN';
    } catch {
      status = 'DOWN';
    }
  ...
  async scanRecentlyPlayed(): Promise<void> {
    ...
    try {
      ...
      for (const variant of variants) {
        ...
        // Pause courte pour ne pas saturer les fournisseurs
