...
export class StreamingController {
  ...
  private async forward(reply: FastifyReply, request: FastifyRequest, context: StreamContext, providerUrl: string, aliasId: string): Promise<FastifyReply> {
    ...
    if (cacheKey) {
      ...
      if (cached) return this.sendBuffered(reply, cached.buffer, cached.contentType, 200, 'HIT');
    }
    ...
    try {
      ...
      if (cacheKey) {
        const staleSegment = this.segmentCache.get(cacheKey);
        if (staleSegment) return this.sendBuffered(reply, staleSegment.buffer, staleSegment.contentType, 200, 'STALE');
      }
      throw error;
    }
  ...
  private pipeSegment(reply: FastifyReply, response: StreamProxyResponse, cacheKey: string | null): FastifyReply {
    ...
    reply.header('cdn-cache-control', SEGMENT_CACHE_CONTROL);
    reply.header('x-mbolo-stream-cache', 'MISS');
    reply.header('x-accel-buffering', 'no');
    reply.status(response.status);
    if (cacheKey) teeSegmentToCache(response.stream, cacheKey, response.contentType, MAX_CACHED_SEGMENT_BYTES);
    abortOnDisconnect(reply, response.stream);
    return reply.send(response.stream);
  }

  private sendBuffered(reply: FastifyReply, buffer: Buffer, contentType: string | null, status: number, cacheStatus = 'HIT'): FastifyReply {
    if (contentType) reply.header('content-type', contentType);
    reply.header('content-length', String(buffer.byteLength));
    reply.header('cache-control', SEGMENT_CACHE_CONTROL);
    reply.header('cdn-cache-control', SEGMENT_CACHE_CONTROL);
    reply.header('x-mbolo-stream-cache', cacheStatus);
    reply.status(status);
  ...
  private sendPlaylistContent(reply: FastifyReply, content: string): FastifyReply {
    ...
    reply.header('cdn-cache-control', PLAYLIST_CACHE_CONTROL);
    reply.header('x-mbolo-stream-cache', 'PLAYLIST');
    reply.header('x-accel-buffering', 'no');
    reply.status(200);
...
function teeSegmentToCache(stream: Readable, key: string, contentType: string | null, maxBytes: number): void {
  const chunks: Buffer[] = [];
  let total = 0;
  let cacheable = true;
  ...
  stream.once('end', () => { if (cacheable && total > 0) streamCacheForCurrentRequest?.(key, Buffer.concat(chunks), contentType); });
  const streamCacheForCurrentRequest = (cacheKey: string, buffer: Buffer, type: string | null): void => { /* replaced by bound cache in bindSegmentCache */ };
}
