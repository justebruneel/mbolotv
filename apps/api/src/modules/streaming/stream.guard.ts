...
export class StreamSessionGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    const request = context
      .switchToHttp()
      .getRequest<FastifyRequest & { streamContext?: StreamContext }>();
    const sessionId = (request.params as { sessionId: string }).sessionId;

    const session = this.streamingService.assertSession(sessionId);
  ...
  private assertRateLimited(
    ...
    if (!result.allowed) {
      ...
      throw new HttpException('Trop de requêtes', 429);
    }
  }
}
