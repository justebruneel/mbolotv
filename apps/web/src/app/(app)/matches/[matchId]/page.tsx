  ...
  return (
    <>
      ...
      <PageHeader
        title={
          <span className="flex items-center gap-2">
            ...
            <Badge tone={match.state === 'LIVE' ? 'danger' : 'accent'} live={match.state === 'LIVE'}>
              {STATE_LABEL[match.state]}
            </Badge>
          </span>
        }
        description={`${match.competition || match.sport} · ${startsAt.toLocaleString('fr-FR', {
          weekday: 'short',
          day: 'numeric',
          month: 'short',
          hour: '2-digit',
      ...
      {match.channels.length > 0 && (
        <div className="mt-5">
          <h2 className="mb-3 text-sm font-semibold text-muted">Chaînes de diffusion</h2>
          <div className="flex flex-wrap gap-2">
            {match.channels.map((channel) => (
              ...
              >
                {channel.name}
                {channel.streamCount > 1 && <span className="opacity-70"> · {channel.streamCount} flux</span>}
              </button>
            ))}
          </div>
          {match.channels.length > 1 && (
            <p className="mt-2 text-xs text-muted">
              Si le flux rame, bascule sur une autre chaîne : elle propose généralement le même match.
            </p>
          )}
        </div>
      )}
